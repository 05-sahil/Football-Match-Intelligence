...
def save_heatmap(pos_a, pos_b, shape, path):
    ...
    for ax,pos,title,cmap in zip(axes,[pos_a,pos_b],
        ...
        ax.axis("off")
    fig.suptitle("FOOTBALL MATCH ANALYTICS — HEATMAPS  |  dev: Sahil dalal",
...
def save_dashboard(hist_spd, hist_act, poss, pt, frame_idx, path):
    ...
    fig.suptitle("FOOTBALL MATCH ANALYTICS — REPORT  |  dev: Sahil dalal",
...
def process(video_path):
    ...
    with tqdm(total=total,unit="fr",ncols=80,colour="cyan") as pbar:
        while True:
            ...
            cv2.putText(canvas,"FOOTBALL MATCH ANALYTICS  v3.1  |  dev: Sahil dalal",
